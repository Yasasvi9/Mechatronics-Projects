...
void initUart0()
{
    ...
    // Configure UART0 to 9600 baud (assuming fcyc = 40 MHz), 8N1 format
    UART0_CTL_R = 0;                                    // turn-off UART0 to allow safe programming
    UART0_CC_R = UART_CC_CS_SYSCLK;                     // use system clock (40 MHz)
    UART0_IBRD_R = 21;                                  // r = 40 MHz / (Nx9600Hz), set floor(r)=260, where N=16
    UART0_FBRD_R = 45;                                  // round(fract(r)*64)=27
...
// Blocking function that returns with serial shellCommand once the buffer is not empty
char getcUart0()
{
